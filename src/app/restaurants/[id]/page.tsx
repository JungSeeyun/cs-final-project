  ...
  return (
    <div className="max-w-2xl mx-auto pb-10">
      ...
      <div className="space-y-2 mb-6">
        {menu.map(item => (
          <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center justify-between hover:shadow-md transition-all">
            ...
            <button onClick={() => addToCart(item.id, item.name)} disabled={adding === item.id}
              className="ml-4 shrink-0 bg-yellow-400 text-gray-900 px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-yellow-500 transition disabled:opacity-50">
              {adding === item.id ? '...' : '담기'}
            </button>
          </div>
        ))}
      ...
      {/* 리뷰 목록만 표시 (작성은 주문완료 후 주문내역에서 가능) */}
      <div className="border-t border-gray-100 pt-6">
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <span className="text-xs text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg">주문 완료 후 리뷰 작성 가능</span>
        </div>
